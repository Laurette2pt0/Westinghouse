...
bureau = os.path.expanduser(r"~/Desktop")
...
def exporter_image():
    ...
    try:
        print(f"chemin : {chemin_image_actuelle}")
        ...
        for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
            ...
            if ratio_px_mm:
                texte = f"{n_elmt}: {(r_img * 2) / ratio_px_mm:.2f} mm"
            else:
        ...
        for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
            ...
            if ratio_px_mm:
                texte = f"{n_elmt}: {(r * 2) / ratio_px_mm:.2f} mm"
            else:
...
def exporter_csv():
    if not cercles and not segments:
        ...
        return
    try:
        # bureau = r"C:\Users\wqp8031\OneDrive - Westinghouse Electric Company LLC\Bureau"
...
def exporter_json():
    ...
    try:
        ...
        data = {"date": datetime.now().isoformat(),
                ...
                "segments": []}
        for i, (cx, cy, r, n_elmt) in enumerate(cercles):
            data["cercles"].append({"nom": n_elmt, "centre_x": cx, "centre_y": cy,
                                    "diametre_px": r*2,
                                    "diametre_mm": (r*2)/ratio_px_mm if ratio_px_mm else None,
                                    "type": "Etalon" if i==cercle_etalon_idx else "Mesure"})
        for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
            longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
            data["segments"].append({"nom": n_elmt, "x1": x1, "y1": y1, "x2": x2, "y2": y2,
...
def exporter_pdf():
    ...
    try:
        ...
        if cercles:
            ...
            data = [["Nom", "Diametre (px)", "Diametre (mm)", "Type"]]
            for i, (cx, cy, r, n_elmt) in enumerate(cercles):
                d_px = r*2
                d_mm = d_px/ratio_px_mm if ratio_px_mm else 0
                data.append([n_elmt, f"{d_px:.1f}", f"{d_mm:.2f}" if ratio_px_mm else "--",
                             "Etalon" if i==cercle_etalon_idx else "Mesure"])
            t = Table(data)
        ...
        if segments:
            ...
            data = [["Nom", "Longueur (px)", "Longueur (mm)", "Type"]]
            for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
                longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
                l_mm = longueur/ratio_px_mm if ratio_px_mm else 0
                data.append([n_elmt, f"{longueur:.1f}", f"{l_mm:.2f}" if ratio_px_mm else "--",
                             "Etalon" if n_elmt == segment_etalon_idx else "Mesure"])
            t = Table(data)
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.grey),
                                   ('ALIGN',(0,0),(-1,-1),'CENTER'),
...
def supprimer_cercle_par_nom():
    """Affiche une fenêtre pour choisir quel cercle supprimer"""
    if not cercles:
        dpg.configure_item("statut", color=(255, 0, 0))
    ...
    with dpg.window(tag="supprimer_cercle_window", label="Supprimer un cercle", width=300, height=200,
        ...
        # Liste des cercles avec leurs noms
        items = []
        for i, (cx, cy, r, nom) in enumerate(cercles):
            if ratio_px_mm:
                diam = f"{(r*2)/ratio_px_mm:.2f} mm"
            else:
                diam = f"{r*2:.1f} px"
            items.append(f"{nom} - {diam}") 
...
def valider_suppression_cercle(nom_selectionne):
    ...
    # Trouver et supprimer le cercle
    for i, (cx, cy, r, nom) in enumerate(cercles):
...
def supprimer_segment_par_nom():
    """Affiche une fenêtre pour choisir quel segment supprimer"""
    if not segments:
        dpg.configure_item("statut", color=(255, 0, 0))
    ...
    with dpg.window(tag="supprimer_segment_window", label="Supprimer un segment", width=300, height=200,
        ...
        for i, (x1, y1, x2, y2, nom) in enumerate(segments):
            longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
            if ratio_px_mm:
                long_str = f"{longueur/ratio_px_mm:.2f} mm"
            else:
                long_str = f"{longueur:.1f} px"
            items.append(f"{nom} - {long_str}")
