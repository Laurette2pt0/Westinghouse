...
def update_resultats():
    ...
    dpg.add_table_column(label="Nom", parent="table_resultats", width_fixed=True, init_width_or_weight=100)  # Changé: 100 au lieu de 20
...
def update_resultats():
    if not dpg.does_item_exist("table_resultats"):
        return
    dpg.delete_item("table_resultats", children_only=True)
    # Utiliser width_fixed=False pour que les colonnes s'adaptent
    dpg.add_table_column(label="Nom", parent="table_resultats", width_fixed=False, init_width_or_weight=1.5)
    dpg.add_table_column(label="Diam/Long", parent="table_resultats", width_fixed=False, init_width_or_weight=1)
    dpg.add_table_column(label="Type", parent="table_resultats", width_fixed=False, init_width_or_weight=0.8)
...
def calibrate():
    ...
    if valeur==ID_CAMERA_PAR_DEFAUT:
        #region chemin des image de distorsion à modifier en cas de besoin
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=260, height=1000):
            ...
            with dpg.table(tag="table_resultats", header_row=True,
               ...
               row_background=True, width=260):  # Changé de 235 à 260
