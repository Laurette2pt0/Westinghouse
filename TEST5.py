...
def exporter_pdf():
    ...
    try:
        ...
        if canal_plus_use_BD_nom and canal_plus_use_BD_cas:
            data = [
                ["Nom du canal", "Classification", "Rapport"],
                [canal_plus_use_BD_nom, canal_plus_use_BD_cas, canal_plus_use_BD_rapport]
            ]
        ...
        if canal_plus_use_CF_nom and canal_plus_use_CF_cas:
            data = [
                ["Nom du canal", "Classification", "Rapport"],
                [canal_plus_use_CF_nom, canal_plus_use_CF_cas, canal_plus_use_CF_rapport]
            ]
            t = Table(data, colWidths=[120, 120, 80])
            t.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.grey),
        ...
        story.append(PageBreak())
               ...
               # ==========================================================
        # IMAGE CORRIGÉE (remplace image brute)
        # ==========================================================
        if img_originale_corrigee is not None:
            try:
                ...
                if success:
                    ...
                    story.append(Paragraph("Image corrigée:", styles['Heading2']))
                    story.append(Spacer(1, 6))
                    story.append(img_pdf)
                    story.append(Spacer(1, 12))
            except Exception as e:
                print(f"ERREUR image corrigée PDF: {e}")
        ...
        if img_analysee is not None:
            try:
                ...
                if success:
                    ...
                    story.append(Paragraph("Image analysée:", styles['Heading2']))
                    story.append(Spacer(1, 6))
                    story.append(img_pdf2)
                    story.append(Spacer(1, 12))
            except Exception as e:
                print(f"ERREUR image analysée PDF: {e}")
...
def update_cas_automatique():
    ...
    try:
        rapport_BD = float(dpg.get_value("input_BD_rapport")) if dpg.get_value("input_BD_rapport") else 0
    except ValueError:
        rapport_BD = 0
    
    try:
        rapport_CF = float(dpg.get_value("input_CF_rapport")) if dpg.get_value("input_CF_rapport") else 0
    except ValueError:
        rapport_CF = 0
...
def sauvegarder_export_complet(sender, app_data):
    ...
    # ==========================================================
    try:
        rapport_BD = float(canal_plus_use_BD_rapport) if canal_plus_use_BD_rapport else 0
    except ValueError:
        rapport_BD = 0
    
    try:
        rapport_CF = float(canal_plus_use_CF_rapport) if canal_plus_use_CF_rapport else 0
    except ValueError:
        rapport_CF = 0
