...
#rajout image des canaux + nom des element que B1, D2... + le dezoom bien + canal de ref 
...
bureau = r"~/Desktop" #~\OneDrive - Westinghouse Electric Company LLC\Bureau"
...
def exporter_image():
    ...
    try:
        nom = f"{bureau}/image_mesures_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        ...
        for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
            ...
            if reference_value is not None and i != cercle_reference_idx:
...
def ouvrir_dossier():
    import subprocess
    subprocess.Popen(f'explorer "{bureau}"')
    dpg.configure_item("statut", color=(0, 255, 0))
    dpg.set_value("statut", f"Bureau ouvert: {bureau}")
...
def exporter_csv():
    if not cercles and not segments:
        ...
        return
    try:
        nom = f"{bureau}/mesures_CAMVIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        with open(nom, 'w', newline='', encoding='utf-8') as f:
            ...
            for i, (cx, cy, r, n_elmt) in enumerate(cercles):
                ...
                if reference_value is not None and i != cercle_reference_idx:
                    ...
                    pourc_str = "--"
                w.writerow([n_elmt, f"{d_px:.1f}", pourc_str, "Reference" if i==cercle_reference_idx else "Mesure"])
            w.writerow([])
            w.writerow(["=== SEGMENTS ==="])
...
def exporter_json():
    if not cercles and not segments:
        ...
        return
    try:
        nom = f"{bureau}/mesures_CAMVIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for i, (cx, cy, r, n_elmt) in enumerate(cercles):
            d_px = r*2
            item = {"nom": n_elmt, "diametre_px": d_px, "type": "Reference" if i==cercle_reference_idx else "Mesure"}
            if reference_value is not None and i != cercle_reference_idx:
...
def exporter_pdf():
    ...
    try:
        ...
        nom = f"{bureau}/rapport_CAMVIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        story.append(Paragraph(f"Camera ID: {ID_CAMERA_PAR_DEFAUT}", styles['Normal']))
...
def afficher_canaux():
    ...
    img_originale = img.flatten().astype(np.float32) / 255.0
    x, y = W-largeur, H-hauteur
    with dpg.texture_registry():
        texture_canaux = dpg.add_static_texture(img.shape[1], img.shape[0], img_originale)
...
def update_display():
    ...
    for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
        ...
        if reference_type == "cercle" and i == reference_idx:
            ...
            label = f"REF: {int(r_img * 2)} px - {n_elmt}"
        else:
    ...
    for i, (x1_img, y1_img, x2_img, y2_img, n_elmt) in enumerate(segments):
        ...
        if reference_type == "segment" and i == reference_idx:
            couleur_bord = (255, 165, 0, 255)
            label = f"REF: {longueur:.1f} px - {n_elmt}"
...
def calibrate():
    ...
    if valeur==ID_CAMERA_PAR_DEFAUT:
        images = glob.glob("distorsion/*.jpg")
    else : 
        dpg.configure_item("statut", color=(255, 0, 0))
...
def callback_fichier(sender, app_data):
    global chemin_image_actuelle, img_originale, texture_minimap
...
def definir_reference():
    ...
    # Priorité au dernier cercle si existant, sinon dernier segment
    if cercles:
        idx = len(cercles) - 1
        cx, cy, r, nom = cercles[idx]
        reference_value = r * 2
        reference_type = "cercle"
        reference_idx = idx
        reference_nom = nom
        dpg.configure_item("statut", color=(0, 255, 0))
        ...
        reference_nom = nom
        dpg.configure_item("statut", color=(0, 255, 0))
        dpg.set_value("statut", f"Reference definie - Segment '{nom}' (Longueur = {reference_value:.1f} px)")
...
def valider(sender, app_data):
    ...
    if not format_valide(nom_element):
        ...
        dpg.set_value("text_popup", "Écris le nom de l'élément :")
...
def valider_suppression_cercle(nom_selectionne):
    global cercles, cercle_etalon_idx, ratio_px_mm, cercle_reference_idx, reference_value
    ...
    for i, (cx, cy, r, nom) in enumerate(cercles):
        if nom == nom_a_supprimer:
            ...
            if cercle_etalon_idx == i:
                ...
                ratio_px_mm = None
                dpg.set_value("label_kp", "Kp = non calibre")
            if cercle_reference_idx == i:
                cercle_reference_idx = None
                reference_value = None
            elif cercle_reference_idx is not None and i < cercle_reference_idx:
                cercle_reference_idx -= 1
            dpg.configure_item("statut", color=(0, 255, 0))
...
with dpg.window(tag="fenetre_principale", width=1920, height=1080):
    ...
    # dpg.add_text("WHE Equipe Instrumentation & Vision ",
    #              color=(180, 180, 180))
    # dpg.add_separator()
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=260, height=1000):
            ...
            dpg.add_combo(items=[ID_CAMERA_PAR_DEFAUT, "Option 2",],label="Camera",callback=calibrate,tag="camera")
