...
#plus la calib ni json (en commentaire)
#v3 : rajout image des canaux + nom des element que B1, D2... + le dezoom bien + canal de ref + bonne couleurs pour les elements
#v4 : ptite couleur joli des cercle et sgment+ exporter image réparé bien
#v5 : rajouter lenregistrement des 3 version dimage dans un dossier joli placé sur le menu + image des canaux pas flou + csv cest vrai tableau mtn + canal de ref que les A en proposition
#v6 : on peut par rajouter des element si on a pas d'image
...
bureau = os.path.expanduser(r"~/Desktop") #~\OneDrive - Westinghouse Electric Company LLC\Bureau"
...
def exporter_image():
    ...
    try:
        ...
        for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
            ...
            cv2.putText(img_export, texte, (int(cx_img - 80), int(cy_img - rayon - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, couleur, 2)
            couleur = []
        ...
        for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
            ...
            cv2.putText(img_export, texte, (int(cx - 80), int(cy - 15)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, couleur, 2)
            couleur = []
        
        info_y = img_export.shape[0] - 20
        if reference_value is not None:
            info = f"CAMVIS - Reference (Canal A): {reference_value:.1f} px"
        elif ratio_px_mm:
            info = f"CAMVIS - Kp = {ratio_px_mm:.4f} px/mm"
        else:
            info = "CAMVIS - Sans reference"
...
def ouvrir_dossier():
    ...
    dpg.configure_item("statut", color=(0, 255, 0))
    dpg.set_value("statut", f"Bureau ouvert: {dossier_export}")
...
def exporter_csv():
    ...
    try:
        ...
        with open(nom, 'w', newline='', encoding='utf-8-sig') as f:
            ...
            if reference_value is not None:
                w.writerow([f"Reference (Canal A): {reference_value:.1f} px"])
            w.writerow([])
            w.writerow(["=== CERCLES ==="])
            w.writerow(["Nom", "Diametre (px)", "% du Canal A", "Type"])
            for i, (cx, cy, r, n_elmt) in enumerate(cercles):
                d_px = r*2
                if reference_value is not None and i != reference_idx:
                    pourc = (d_px / reference_value) * 100
                    pourc_str = f"{pourc:.1f}%"
                else:
                    pourc_str = "--"
                w.writerow([n_elmt, f"{d_px:.1f}", pourc_str, "Reference" if (i == reference_idx and reference_type == "cercle") else "Mesure"])
            w.writerow([])
            w.writerow(["=== SEGMENTS ==="])
            w.writerow(["Nom", "Longueur (px)", "Type"])
            for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
                longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
                w.writerow([n_elmt, f"{longueur:.1f}", "Mesure"])
        dpg.configure_item("statut", color=(0, 255, 0))
...
def exporter_pdf():
    ...
    try:
        ...
        nom = f"{dossier_export}/rapport_CAMVIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        if os.path.exists(chemin_logo):
            try:
                logo = Image(chemin_logo, width=100, height=100)
                story.append(logo)
        ...
        story.append(Paragraph(f"Camera ID: {ID_CAMERA}", styles['Normal']))
        story.append(Paragraph(f"Image: {os.path.basename(chemin_image_actuelle) if chemin_image_actuelle else 'N/A'}", styles['Normal']))
        if reference_value is not None:
            story.append(Paragraph(f"Canal A (Reference): {reference_value:.1f} px", styles['Normal']))
        story.append(Spacer(1, 12))
        ...
        # Cercles avec rapports
        if cercles:
            story.append(Paragraph("Cercles mesures:", styles['Heading2']))
            data = [["Nom", "Rapport", "Type"]]
            max_rapport = 0
            max_nom = ""
            max_diam = 0
            
            for i, (cx, cy, r, n_elmt) in enumerate(cercles):
                if i == reference_idx and reference_type == "cercle":
                    rapport_str = "REFERENCE"
                    type_str = "CANAL A"
                elif reference_value is not None:
                    rapport = (r*2) / reference_value
                    rapport_str = f"x{rapport:.2f}"
                    type_str = "Mesure"
                    if rapport > max_rapport:
                        max_rapport = rapport
                        max_nom = n_elmt
                        max_diam = r*2
                else:
                    rapport_str = f"{r*2:.1f} px"
                    type_str = "Cercle"
                
                data.append([n_elmt, rapport_str, type_str])
            ...
            if max_rapport > 0:
                ...
                story.append(Paragraph(f"Le cercle '{max_nom}' est le plus grand par rapport au Canal A", styles['Normal']))
                story.append(Paragraph(f"Diametre: {max_diam:.1f} px (soit x{max_rapport:.2f} le Canal A)", styles['Normal']))
                story.append(Spacer(1, 12))
        ...
        signature_data = [["Date:", "____________________"], ["Signature:", "____________________"]]
        ...
        # Pied de page
        story.append(Paragraph("CAMVIS - WHE Equipe Instrumentation & Vision", styles['Normal']))
        story.append(Paragraph(f"Document genere le {datetime.now().strftime('%d/%m/%Y')}", styles['Normal']))
...
def update_resultats():
    ...
    dpg.add_table_column(label="Valeur / Rapport", parent="table_resultats", width_fixed=True, init_width_or_weight=90)
    ...
    for i, (cx, cy, r, n_elmt) in enumerate(cercles):
        ...
        if reference_type == "cercle" and i == reference_idx:
            rapport_str = f"{diam_px:.1f} px (REF)"
            type_str = "CANAL A"
        elif reference_value is not None and reference_type == "cercle":
            rapport = diam_px / reference_value
            rapport_str = f"x{rapport:.2f}"
            type_str = "Mesure"
        else:
            rapport_str = f"{diam_px:.1f} px"
            type_str = "Cercle"
        ...
        with dpg.table_row(parent="table_resultats"):
            ...
            if reference_type == "cercle" and i == reference_idx:
                dpg.add_text("REFERENCE", color=(255, 165, 0))
            elif i == cercle_etalon_idx:
    ...
    for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
        ...
        if reference_type == "segment" and i == reference_idx:
            rapport_str = f"{longueur:.1f} px (REF)"
            type_str = "REFERENCE"
        elif reference_value is not None and reference_type == "segment":
            rapport = longueur / reference_value
            rapport_str = f"x{rapport:.2f}"
            type_str = "Mesure"
        else:
            rapport_str = f"{longueur:.1f} px"
            type_str = "Segment"
        ...
        with dpg.table_row(parent="table_resultats"):
            ...
            if reference_type == "segment" and i == reference_idx:
                dpg.add_text("REFERENCE", color=(255, 165, 0))
            elif n_elmt == segment_etalon_idx:
                dpg.add_text("Etalon", color=(255, 165, 0))
            else:
                dpg.add_text("Mesure", color=(100, 255, 100))
...
with dpg.window(tag="fenetre_principale", width=1920, height=1080):
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=260, height=1000):
            ...
            dpg.add_combo(items=["CAM_001_eau", "CAM_001_air",],label="Camera",callback=calibrate,tag="camera")
