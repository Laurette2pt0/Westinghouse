...
ptit_cercle = 0 #numéro des petits cercles créés
ptit_sgmt = 0 #numéro des segments créés
...
cx, cy, w_g, h_g = 0, 0, 0, 0 #coordonnées image des canaux grande
texture_canaux_petit = None #tag de l'image des canaux en bas à droite
texture_canaux_grand = None #tag de l'image des canaux aggrandit
...
try:
    ...
    info_affaire = donnees["info_affaire"]  # Nom de l'affaire
    info_palier = donnees["info_palier"]  # Nom du palier
    info_central = donnees["info_central"]  # Nom de la centrale
    info_tranche = donnees["info_tranche"] # Numéro de tranche
    info_visite = donnees["info_visite"] # Numéro de visite

except FileNotFoundError:
    # Valeurs par défaut si aucune sauvegarde n'existe
    info_affaire = ""  # Nom de l'affaire
    info_palier = ""  # Nom du palier
    info_central = ""  # Nom de la centrale
    info_tranche = ""  # Numéro de tranche
...
# Informations spécifiques pour l'export PDF (NOUVELLE POPUP)
...
dossier_export = os.path.join(bureau, "CAMVIS_Export") # Dossier d'export
os.makedirs(dossier_export, exist_ok=True)  # Création du dossier si inexistant
nom_dossier = ""  # Nom du sous-dossier pour l'export
nom_fichier = ""  # Nom du fichier PDF
# ─────────────────────────────────────────
# region ZOOM & PAN
# ─────────────────────────────────────────
zoom_level = 1.0  # Niveau de zoom actuel
ZOOM_MIN   = 0.81  # Zoom minimum
ZOOM_MAX   = 10.0  # Zoom maximum
ZOOM_STEP  = 0.15  # Pas de zoom

view_x = 0.0  # Position X de la vue
view_y = 0.0  # Position Y de la vue

tex_src_x = 0  # Coordonnée X source de la texture
tex_src_y = 0  # Coordonnée Y source de la texture

pan_actif  = False  # Indique si le mode pan est actif
pan_last_x = 0.0  # Dernière position X de la souris en pan
pan_last_y = 0.0  # Dernière position Y de la souris en pan

MINI_W = 260  # Largeur de la mini-carte
MINI_H = 0  # Hauteur de la mini-carte (calculée automatiquement)
MINI_X = W - MINI_W - 10  # Position X de la mini-carte
MINI_Y = 10  # Position Y de la mini-carte

img_originale = None  # Image originale non corrigée
img_originale_corrigee = None  # Image corrigée
name_img = ""  # Nom de l'image sans extension
name_img_corrigee = ""  # Nom de l'image corrigée
...
# ─────────────────────────────────────────
# region EXPORT IMAGE, CSV, JSON, PDF
# ─────────────────────────────────────────
...
def exporter_image():
    ...
    try:
        ...
        for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
            ...
            #print(f"clr = {clr}, et couler = {couleur} pour {label}")
            ...
            # if reference_value is not None and i != reference_idx:
            #     pourc = (r_img * 2 / reference_value) * 100
            #     texte = f"{n_elmt}: {pourc:.1f}%"
            # else:
            #     texte = f"{n_elmt}: {int(r_img * 2)} px"
            
            cv2.putText(img_export, label, (int(cx_img - 80), int(cy_img - rayon - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, couleur, 2)
            couleur = []
        ...
        for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
            ...
            cv2.circle(img_export, (int(cx), int(cy)), 5, couleur, -1) #banc

            # if reference_value is not None and i != reference_idx:
            #     pourc = (r * 2 / reference_value) * 100
            #     texte = f"{n_elmt}: {pourc:.1f}%"
            # else:
            #     texte = f"{n_elmt}: {int(r * 2)} px"
...
def exporter_csv():
    ...
    try:
        ...
        with open(nom, 'w', newline='', encoding='utf-8-sig') as f:
            ...
            if reference_value is not None:
                w.writerow([f"Reference : {reference_nom}"]) #: {reference_value:.1f} px
            w.writerow([])
            w.writerow(["=== CERCLES ==="])
            w.writerow(["Nom", "Cas", "Type"]) #"Diametre (px), "% du Canal A""
            for i, (cx, cy, r, n_elmt) in enumerate(cercles):
                d_px = r*2
                rapport_str, type_str = choose_cas(n_elmt, d_px, "cercle", i)
                w.writerow([n_elmt, rapport_str, type_str]) #f"{d_px:.1f}", pourc_str, "Reference" if (i == reference_idx and reference_type == "cercle") else "Mesure"
            w.writerow([])
            w.writerow(["=== SEGMENTS ==="])
...
def exporter_pdf():
    ...
    try:
        ...
        tableau_donnees = []
        tableau_donnees.append(["Date:", datetime.now().strftime('%d/%m/%Y %H:%M:%S')])
        ...
        if img_originale is not None:
            try:
                img_bgr = cv2.cvtColor(img_originale.copy(), cv2.COLOR_RGBA2BGR)
                h, w_img = img_bgr.shape[:2]
                max_width = 500
                if w_img > max_width:
                    new_h = int(h * max_width / w_img)
                    img_bgr = cv2.resize(img_bgr, (max_width, new_h))
                img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
                pil_img = ImageReader(img_rgb)
                img_pdf = Image(pil_img, width=img_bgr.shape[1], height=img_bgr.shape[0])
        ...
        # Trouver LE PLUS GRAND cercle pour chaque type (B/D et C/F)
        plus_grand_BD = None
        plus_grand_CF = None
        max_rapport_BD = 0
        max_rapport_CF = 0
        ...
        if plus_grand_BD:
            data = [["Nom", "Classification"]]
            data.append([plus_grand_BD["nom"], plus_grand_BD["cas"]])
            t = Table(data)
            ...
            story.append(Paragraph("Aucun element depassant le seuil pour les canaux B/D", styles['Normal']))
        story.append(Spacer(1, 12))
        
        # Tableau pour les canaux C/F
        story.append(Paragraph("Canaux C/F - Plus grand element:", styles['Heading2']))
        if plus_grand_CF:
            data = [["Nom", "Classification"]]
            data.append([plus_grand_CF["nom"], plus_grand_CF["cas"]])
            t = Table(data)
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.grey),
                                   ('ALIGN',(0,0),(-1,-1),'CENTER'),
                                   ('GRID',(0,0),(-1,-1),1,colors.black)]))
            ...
            story.append(Paragraph("Aucun element depassant le seuil pour les canaux C/F", styles['Normal']))
        story.append(Spacer(1, 12))
        ...
        signature_data = [["Analyse:", "____________________"], ["Controle technique :", "____________________"]]
        signature_table = Table(signature_data, colWidths=[100, 300])
        signature_table.setStyle(TableStyle([('FONTNAME', (0,0), (-1,-1), 'Helvetica'),
                                             ('FONTSIZE', (0,0), (-1,-1), 10),
                                             ('VALIGN', (0,0), (-1,-1), 'MIDDLE')]))
...
def afficher_canaux():
    ...
    # x, y = W-largeur, H-hauteur
    # with dpg.texture_registry():
    #     texture_canaux = dpg.add_static_texture(img_ptite.shape[1], img_ptite.shape[0], image)
    # 3. Afficher sur le drawlist
    # xc, yc, xcf, ycf = x, y, x + largeur, y + hauteur
    dpg.draw_image(texture_canaux_petit, (xc, yc), (xcf, ycf), parent="drawlist_principal")
    # print(survol_canaux)
    if survol_canaux:
        # Afficher en grand au centre
        # grande_taille = 970
        # img_grande = cv2.resize(img, (grande_taille, grande_taille), interpolation=cv2.INTER_LINEAR)
        # h_g, w_g = img_grande.shape[:2]
        # img_data = img_grande.flatten().astype(np.float32) / 255.0
        # with dpg.texture_registry():
        #     tex = dpg.add_static_texture(w_g, h_g, img_data)
        # cx = W // 2 - w_g // 2
        # cy = H // 2 - h_g // 2
...
def choose_cas(n_elmt, d, type, i):
    ...
    if i == reference_idx and  reference_type == type :
        ...
        rapport_str = f"cas 0"
        type_str = "Mesure"
    elif reference_value is not None and BD and (1.5 < (d / reference_value) <= 2):
        rapport_str = f"cas 1"
        type_str = "Mesure"
    elif reference_value is not None and BD and (2 < (d / reference_value) <= 2.5) :
        rapport_str = f"cas 2"
        type_str = "Mesure"
...
def update_display():
    ...
    for i, (cx_img, cy_img, r_img, n_elmt) in enumerate(cercles):
        ...
        # print(f"n_element = {nom_element} ; radius = {r_img} et r_screen = {r_screen}")
    ...
    with dpg.window(tag="reference_window", label="Choisir canal de reference", width=300, height=200,
        ...
        for i, (cx, cy, r, nom) in enumerate(cercles):
            if nom.startswith('A') :
                if reference_value is not None and i != reference_idx:
                    pourc = (r*2 / reference_value) * 100
                    items.append(f"{nom} - cercle") #- {pourc:.1f}%
                else:
                    items.append(f"{nom} - cercle") #- {r*2:.1f} px
        
        for i, (x1, y1, x2, y2, nom) in enumerate(segments):
            if nom.startswith('A') :
                longueur = math.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                if reference_value is not None and i != reference_idx:
                    pourc = (longueur / reference_value) * 100
                    items.append(f"{nom} - segment")# - {pourc:.1f}%
                else:
                    items.append(f"{nom} - segment")# - {longueur:.1f} px
    ...
    if reference_type == "segment" :
        for i, (x1, y1, x2, y2, nom) in enumerate(segments):
            if nom == nom_ref:
                ...
                reference_nom = nom
                dpg.configure_item("statut", color=(0, 255, 0))
                dpg.set_value("statut", f"Reference definie - Segment '{nom_ref}'") # (Diametre = {reference_value:.1f} px) 
                break
    else : #cercle
        for i, (cx, cy, r, nom) in enumerate(cercles):
            if nom == nom_ref:
                ...
                reference_nom = nom
                dpg.configure_item("statut", color=(0, 255, 0))
                dpg.set_value("statut", f"Reference definie - Cercle '{nom}'") # (Diametre = {reference_value:.1f} px)
                break
...
def ouvrir_popup(Dist):
    ...
    diametre = Dist
    # print("POPUP")
    with dpg.window(label="Saisie utilisateur", modal=True, tag="fenetre_saisie", pos=(dpg.get_viewport_width()//2 - 150, dpg.get_viewport_height()//2 - 100)) :
...
def valider(sender, app_data):
    ...
    if not format_valide(nom_element):
        ...
        return  # ← on ne ferme PAS la popup, l'utilisateur doit corriger
    dpg.set_value("input_texte", "")
    dpg.hide_item("fenetre_saisie") #la c'est bn, on peut fermer la popup
    if reference_value is not None:
        pourc = (diametre*2 / reference_value) * 100
        dpg.configure_item("statut", color=(0, 255, 0))
        dpg.set_value("statut", f"{nom_element} ajoute") #; {pourc:.1f}% du Canal A
    elif ratio_px_mm:
        dpg.configure_item("statut", color=(0, 255, 0))
...
def on_mouse_click(sender, app_data):
    global point1_temp, cercles, segments, view_x, view_y, temp_cx, temp_cy, temp_x1, temp_y1, temp_x2, temp_y2, obj,reference_value, ptit_cercle, ptit_sgmt
    if not dpg.is_item_hovered("drawlist_principal"):
        return
    if img_originale_corrigee is None :
        dpg.configure_item("statut", color=(255, 0, 0))
        dpg.set_value("statut", f"Erreur : Charger une image")
        return
...
def on_mouse_move(sender, app_data):
    global pan_last_x, pan_last_y, view_x, view_y, survol_canaux, point1_temp
    ...
    if dpg.is_item_hovered("drawlist_principal"):
        ...
        # Vérifie uniquement si la souris est sur la zone exacte de l'image canaux
    ...
    if (not mode_dessin_cercle and not mode_dessin_segment) or (point1_temp is None and reference_value is None):
        return
    if not dpg.is_item_hovered("drawlist_principal"):
        return
    ...
    if point1_temp is not None :
        ...
        x1,y1 = 0, 0
    # print(x1,y1,point1_temp)
    # Apercue du segment  en cours de création
    if mode_dessin_segment and point1_temp is not None and point1_temp is not None:
        update_display()
        ...
        #     pourc = (longueur / reference_value) * 100
        #     dpg.configure_item("statut", color=(0, 255, 0))
        #     dpg.set_value("statut", f"Longueur temporaire = {pourc:.1f}% du Canal A")
        # elif ratio_px_mm:
        #     dpg.configure_item("statut", color=(0, 255, 0))
    ...
    # Apercue du cercle en cours de création
        ...
        if reference_value is None and point1_temp is not None:
            ...
            update_display()
            # print("il est ou mon apercu ??")
            centre = (mx,my)
            radius = (reference_value/2) * zoom_level
            #print(f"radius temporaire = {radius}")
            dpg.draw_line((mx-radius, my), (mx+radius, my), color=(0, 200, 255, 180), thickness=1, parent="drawlist_principal")
            dpg.draw_circle(centre, radius, color=(0, 200, 255, 200), fill=(0, 200, 255, 30), thickness=2, parent="drawlist_principal")
        
        # r = math.sqrt((ix - x1)**2 + (iy - y1)**2) / 2
        # if reference_value is not None:
        #     pourc = (r*2 / reference_value) * 100
        #     dpg.configure_item("statut", color=(0, 255, 0))
        #     dpg.set_value("statut", f"Diametre temporaire = {pourc:.1f}% du Canal A")
        # elif ratio_px_mm:
        #     dpg.configure_item("statut", color=(0, 255, 0))
...
def clear_cercles():
    ...
    cercles = []
    point1_temp = None
    # Si la référence était un cercle, la réinitialiser
...
def clear_segments():
    ...
    segments = []
    point1_temp = None
    # Si la référence était un segment, la réinitialiser
    if reference_type == "segment":
        reference_idx = None
        reference_type = None
        reference_value = None
...
def supprimer_cercle_par_nom():
    ...
    with dpg.window(tag="supprimer_cercle_window", label="Supprimer un cercle", width=300, height=200,
        ...
        for i, (cx, cy, r, nom) in enumerate(cercles):
            if reference_value is not None and i != reference_idx:
                pourc = (r*2 / reference_value) * 100
                items.append(f"{nom}")#- {pourc:.1f}%
            else:
                items.append(f"{nom}")#- {r*2:.1f} px
...
def valider_suppression_cercle(nom_selectionne):
    ...
    nom_a_supprimer = nom_selectionne#.split(" - ")[0]
    ...
    for i, (cx, cy, r, nom) in enumerate(cercles):
        if nom == nom_a_supprimer:
            ...
            if reference_idx == i and reference_type =="cercle":
                ...
                reference_idx -= 1
            dpg.configure_item("statut", color=(0, 255, 0))
            dpg.set_value("statut", f"Cercle '{nom_a_supprimer}' supprimé")
            break
...
def supprimer_segment_par_nom():
    ...
    with dpg.window(tag="supprimer_segment_window", label="Supprimer un segment", width=300, height=200,
        ...
        for i, (x1, y1, x2, y2, nom) in enumerate(segments):
            longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
            if ratio_px_mm:
                items.append(f"{nom}") # - {longueur/ratio_px_mm:.2f} mm
            else:
                items.append(f"{nom}") # - {longueur:.1f} px
    ...
    for i, (x1, y1, x2, y2, nom) in enumerate(segments):
        if nom == nom_a_supprimer:
            ...
            if segment_etalon_idx == nom:
                ...
                # dpg.set_value("label_kp", "Kp = non calibre")
            dpg.configure_item("statut", color=(0, 255, 0))
            dpg.set_value("statut", f"Segment '{nom_a_supprimer}' supprimé")
            break
...
def sauvegarder():
    ...
    donnees_pv = {
        "info_affaire" : info_affaire,  # Nom de l'affaire
        "info_palier" : info_palier,  # Nom du palier
        "info_central" : info_central,  # Nom de la centrale
        "info_tranche" : info_tranche,  # Numéro de tranche
        "info_visite" : info_visite # Numéro de visite
    }
    with open(fichier_donnee, "w", encoding="utf-8") as f:
        json.dump(donnees_pv, f, indent=4)
...
def ouvrir_popup_pdf():
    ...
    with dpg.window(tag="popup_pdf_infos", label="Export PDF - Informations", width=420, height=300,
        ...
        with dpg.group(horizontal=False):
            ...
            # Chaque input avec label à gauche
            dpg.add_text("Chrono:")
            dpg.add_input_text(tag="pdf_chrono", width=350, default_value=pdf_chrono)
            dpg.add_spacer(height=5)
            
            dpg.add_text("Revision:")
            dpg.add_input_text(tag="pdf_revision", width=350, default_value=pdf_revision)
            dpg.add_spacer(height=5)
            
            dpg.add_text("Reference GDG:")
            dpg.add_input_text(tag="pdf_ref_gdg", width=350, default_value=pdf_ref_gdg)
            dpg.add_spacer(height=5)
            
            dpg.add_text("Carte:")
            dpg.add_input_text(tag="pdf_carte", width=350, default_value=pdf_carte)
            dpg.add_spacer(height=20)
            
            with dpg.group(horizontal=True):
                dpg.add_button(label="Exporter PDF", callback=sauvegarder_pdf_infos, width=150)
                dpg.add_button(label="Annuler", callback=fermer_popup_pdf, width=120)
...
with dpg.window(tag="fenetre_principale", width=1920, height=1080):
    ...
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(width=260, height=1000):
            ...
            dpg.add_combo(items=["CAM_GDG_v1_eau", "CAM_GDG_v1_air",],default_value = "CAM_GDG_v1_eau", label="Camera",callback=calibrate,tag="camera")
