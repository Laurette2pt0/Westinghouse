...
#         return
#     try:
#         nom = f"{dossier_export}/mesures_CAMVIS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
#         for i, (cx, cy, r, n_elmt) in enumerate(cercles):
#             d_px = r*2
#             item = {"nom": n_elmt, "diametre_px": d_px, "type": "Reference" if (i==reference_idx and reference_type == "cercle") else "Mesure"}
#             if reference_value is not None and i != reference_idx:
#                 item["pourcentage"] = (d_px / reference_value) * 100
#             data["cercles"].append(item)
#         for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
#             longueur = math.sqrt((x2-x1)**2 + (y2-y1)**2)
#             data["segments"].append({"nom": n_elmt, "longueur_px": longueur, "type": "Mesure"})
#         with open(nom, 'w', encoding='utf-8') as f:
#             json.dump(data, f, indent=4, ensure_ascii=False)
#         dpg.configure_item("statut", color=(0, 255, 0))
#         dpg.set_value("statut", f"JSON exporte sur le Bureau: {nom}")
#     except Exception as e:
#         dpg.configure_item("statut", color=(255, 0, 0))
