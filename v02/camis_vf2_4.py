...
def ouvrir_popup(Dist):
    ...
    diametre = Dist
    # print("POPUP")
    with dpg.window(label="Saisie utilisateur", modal=True, tag="fenetre_saisie", pos=(dpg.get_viewport_width()//2 - 150, dpg.get_viewport_height()//2 - 100), on_close=annuler_saisie) :
...
"""Eviter les beugs l'orque l'utilisateur appuie sur la croix"""
def annuler_saisie():
    global point1_temp
    point1_temp = None
...
"""Savoir si deux ptits cercles se chevuache"""
...
def on_mouse_move(sender, app_data):
    ...
    # Apercue du cercle en cours de création
        ...
        if reference_value is None and point1_temp is not None:
            ...
            couleur = (255, 0, 0, 200) if cercle_chevauche(ix,iy) else (0, 200, 255, 200)
            fill = (255,0,0,30) if cercle_chevauche(ix,iy) else (0,200,255,30)
