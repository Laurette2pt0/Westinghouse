...
# ─────────────────────────────────────────
version = "1.0.1.0" #version du logiciel
...
def exporter_image():
    ...
    try:
        nom = f"{nom_dossier}/{name_img}_analysee.bmp"
        img_export_a = creer_image_analysee()
        cv2.imwrite(nom, img_export_a)

        full_path = f"{nom_dossier}/{name_img}_corrigee.bmp"
        img_export_c = cv2.cvtColor(img_originale_corrigee.copy(), cv2.COLOR_RGBA2BGR)
        cv2.imwrite(full_path, img_export_c)

        full_path = f"{nom_dossier}/{name_img}.bmp"
        img_export = cv2.cvtColor(img_originale.copy(), cv2.COLOR_RGBA2BGR)
        cv2.imwrite(full_path, img_export)
...
def ouvrir_popup(Dist):
    ...
    with dpg.window(label="Saisie utilisateur", modal=True, tag="fenetre_saisie",
                    ...
                    on_close=annuler_saisie, width=350, height=50):
