...
canal_plus_abime_BD_nom = ""  # Nom du canal B/D le plus abîmé
canal_plus_abime_BD_cas = ""  # Cas du canal B/D le plus abîmé
canal_plus_abime_CF_nom = ""  # Nom du canal C/F le plus abîmé
canal_plus_abime_CF_cas = ""  # Cas du canal C/F le plus abîmé
...
bureau = os.path.expanduser(r"~\OneDrive - Westinghouse Electric Company LLC\Bureau")
...
def creer_image_analysee():
    ...
    for i, (x1, y1, x2, y2, n_elmt) in enumerate(segments):
        ...
        for j in range(0, 3):
            couleur.append(clr[2 - j])
        cv2.line(img_export, (int(x1), int(y1)), (int(x2), int(y2)), couleur, 1)  # Trait fin
        cx = (x1 + x2) / 2
        cy = (y1 + y2) / 2
...
def exporter_pdf():
    ...
    try:
        ...
        story.append(PageBreak())
         ...
         # Tableau pour les canaux B/D - avec saisie manuelle
        story.append(Paragraph("Canaux B/D - Canal le plus abîmé:", styles['Heading2']))
        story.append(Spacer(1, 6))
        if canal_plus_abime_BD_nom and canal_plus_abime_BD_cas:
            data = [["Nom du canal", "Classification"], [canal_plus_abime_BD_nom, canal_plus_abime_BD_cas]]
            ...
            story.append(Paragraph("Aucun canal B/D abîmé renseigné", styles['Normal']))
        story.append(Spacer(1, 12))

        # Tableau pour les canaux C/F - avec saisie manuelle
        story.append(Paragraph("Canaux C/F - Canal le plus abîmé:", styles['Heading2']))
        story.append(Spacer(1, 6))
        if canal_plus_abime_CF_nom and canal_plus_abime_CF_cas:
            data = [["Nom du canal", "Classification"], [canal_plus_abime_CF_nom, canal_plus_abime_CF_cas]]
            t = Table(data, colWidths=[150, 150])
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.grey),
                                   ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke),
                                   ('ALIGN',(0,0),(-1,-1),'CENTER'),
            ...
            story.append(Paragraph("Aucun canal C/F abîmé renseigné", styles['Normal']))
        story.append(Spacer(1, 12))
        story.append(PageBreak())
        ...
        if img_originale is not None:
            try:
                ...
                if success:
                    ...
                    story.append(Paragraph("Image brute:", styles['Heading2']))
                    story.append(Spacer(1, 6))
                    story.append(img_pdf)
                    story.append(Spacer(1, 12))
            except Exception as e:
                print(f"ERREUR image brute PDF: {e}")
...
def on_mouse_click(sender, app_data):
    global point1_temp, cercles, segments, view_x, view_y, temp_cx, temp_cy, temp_x1, temp_y1, temp_x2, temp_y2, obj, reference_value, ptit_cercle, ptit_sgmt, premier_segment
    if not dpg.is_item_hovered("drawlist_principal"):
        return
    if img_originale_corrigee is None:
        dpg.configure_item("statut", color=(255, 0, 0))
        ...
        if mode_dessin_segment and 0 <= mx <= W and 0 <= my <= H:
            if point1_temp is None:
                ...
                if premier_segment is None:
                    ...
                    # ===== DEUXIÈME SEGMENT (PERPENDICULAIRE PASSANT PAR LE MILIEU) =====
                    ref_x1, ref_y1, ref_x2, ref_y2 = premier_segment
                    
                    # Calculer le MILIEU du premier segment
                    milieu_x = (ref_x1 + ref_x2) / 2
                    milieu_y = (ref_y1 + ref_y2) / 2
                    
                    # Vecteur du premier segment
                    ref_dx = ref_x2 - ref_x1
                    ref_dy = ref_y2 - ref_y1
                    
                    # Direction perpendiculaire au premier segment
                    dx_perp = -ref_dy
                    dy_perp = ref_dx
                    ...
                    if norm_perp > 0:
                        ...
                        # Direction indiquée par l'utilisateur (pour la longueur)
                        dx_user = x2 - x1
                        dy_user = y2 - y1
                        longueur_user = math.sqrt(dx_user**2 + dy_user**2)
                        
                        if longueur_user > 0:
                            # Le segment perpendiculaire est centre sur le milieu du premier segment
                            # et s'etend symetriquement de chaque cote : il PASSE PAR le milieu au lieu
                            # de simplement en partir dans une seule direction
                            demi_longueur = longueur_user / 2
                            x_perp1 = milieu_x - dx_perp_norm * demi_longueur
                            y_perp1 = milieu_y - dy_perp_norm * demi_longueur
                            x_perp2 = milieu_x + dx_perp_norm * demi_longueur
                            y_perp2 = milieu_y + dy_perp_norm * demi_longueur
                            ...
                            segments.append((x_perp1, y_perp1, x_perp2, y_perp2, nom_element))
                            ptit_sgmt += 1
                            ...
                            dpg.configure_item("statut", color=(255, 0, 0))
                            dpg.set_value("statut", "Erreur : points trop proches")
                    else:
                        dpg.configure_item("statut", color=(255, 0, 0))
                        dpg.set_value("statut", "Erreur : segment de référence invalide")
            return
...
def on_mouse_move(sender, app_data):
    global pan_last_x, pan_last_y, view_x, view_y, survol_canaux, point1_temp, premier_segment
    ...
    # APERÇU DU SEGMENT
    # ==========================================================
    if mode_dessin_segment and point1_temp is not None:
        update_display()
        sx1, sy1 = image_to_screen(x1, y1)
        sx2, sy2 = image_to_screen(ix, iy)
        
        # CORRECTION : Ce bloc doit être indenté sous le if ci-dessus
        if premier_segment is not None:
            ...
            milieu_x = (ref_x1 + ref_x2) / 2
            milieu_y = (ref_y1 + ref_y2) / 2
            smilieu_x, smilieu_y = image_to_screen(milieu_x, milieu_y)
            
            # Afficher le milieu (petit cercle)
            ...
            if norm_perp > 0 and (ref_dx**2 + ref_dy**2) > 0:
                ...
                if longueur_user > 0:
                    ...
                    dpg.draw_line((sx1, sy1), (sx2, sy2), color=(0, 200, 255, 150), thickness=1, parent="drawlist_principal")
...
def ouvrir_popup_export_complet():
    ...
    global canal_plus_abime_BD_nom, canal_plus_abime_BD_cas
    global canal_plus_abime_CF_nom, canal_plus_abime_CF_cas
    
    if dpg.does_item_exist("popup_export_complet"):
        dpg.delete_item("popup_export_complet")
    
    with dpg.window(tag="popup_export_complet", label="Export du rapport", width=480, height=520,
                    pos=(dpg.get_viewport_width()//2 - 240, dpg.get_viewport_height()//2 - 260),
                    modal=True, show=True, no_resize=True):
        ...
        # Section 2: Canaux abîmés B/D
        dpg.add_text(" CANAUX B/D - Canal le plus abîmé ", color=(255, 165, 0))
        dpg.add_spacer(height=5)
        
        dpg.add_text("Nom du canal B/D:")
        dpg.add_input_text(tag="input_BD_nom", width=400, default_value=canal_plus_abime_BD_nom)
        dpg.add_spacer(height=5)
        
        dpg.add_text("Cas (ex: Cas 1, 2 ou 3):")
        dpg.add_input_text(tag="input_BD_cas", width=400, default_value=canal_plus_abime_BD_cas)
        dpg.add_spacer(height=10)
        
        # Section 3: Canaux abîmés C/F
        dpg.add_text(" CANAUX C/F - Canal le plus abîmé ", color=(255, 165, 0))
        dpg.add_spacer(height=5)
        
        dpg.add_text("Nom du canal C/F:")
        dpg.add_input_text(tag="input_CF_nom", width=400, default_value=canal_plus_abime_CF_nom)
        dpg.add_spacer(height=5)
        
        dpg.add_text("Cas (ex: Cas 1):")
        dpg.add_input_text(tag="input_CF_cas", width=400, default_value=canal_plus_abime_CF_cas)
        dpg.add_spacer(height=15)
...
def sauvegarder_export_complet(sender, app_data):
    ...
    canal_plus_abime_BD_nom = dpg.get_value("input_BD_nom")
    canal_plus_abime_BD_cas = dpg.get_value("input_BD_cas")
    canal_plus_abime_CF_nom = dpg.get_value("input_CF_nom")
    canal_plus_abime_CF_cas = dpg.get_value("input_CF_cas")
